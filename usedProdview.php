...
return false;		
	...
	function s1() {
       ...
       window.location.href="usedproductview.php";
        }
      function demo1(b1) {
		
    var elem = document.getElementById("b1").value;

       window.location.href="displayused.php?value="+elem;
	   return false;
        }
...
<label><h3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Enter used  Category</h3></label>
&nbsp;&nbsp;&nbsp;
...
<?php
 $sql5="select distinct catg from used_product";
...
<?php
 ...
 if(isset($_POST['s1']))
 {
	 $comp=$_POST['cat'];
	 $_SESSION['comp']=$comp;
    $sql="select id,img,name from used_product where catg='$comp'";
	}
 else
 {

 
	 $sql="select id,img,name from used_product";
 }
...
if(!empty($_SESSION['comp']))
	...
	if(empty($_SESSION['comp']))
	$sql="select id,img,name from used_product limit $cnt,12";
else
	$sql="select id,img,name from used_product where catg='$var' limit $cnt,12";
...
if(!empty($_SESSION['comp']))
	...
	$sql="select id,img,name from used_product where catg='$var1' limit $cnt,12";
		if ($result=mysqli_query($con,$sql))
		{
			// Return the number of rows in result set
			$rowcount=mysqli_num_rows($result);
...
if(isset($_POST['s2']))
 {
	 ...
	 $frm=$_POST['t2'];
	 $to=$_POST['t3'];
	$sql="select id,img,name from used_product where catg='$comp' and price between $frm and $to";
 }
